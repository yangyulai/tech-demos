...
using RequestHandler = std::function<http::response<http::string_body>(boost::urls::url_view, unsigned)>;
...
private:
    ...
    void do_session(tcp::socket socket) {
        ...
        if (ec)
            return;
        auto result = boost::urls::parse_uri_reference(req.target());
        if (!result.has_value()) {
            http::response<http::string_body> res{ http::status::bad_request, req.version() };
            res.prepare_payload();
        }
        boost::urls::url_view u = result.value();
        auto key = RouteKey{ req.method(), std::string(u.path()) };
        auto it = routes_.find(key);
        http::response<http::string_body> res;
        std::string_view query = u.query();
        if (it != routes_.end()) {
            res = it->second(u, req.version());
        }
        else {
