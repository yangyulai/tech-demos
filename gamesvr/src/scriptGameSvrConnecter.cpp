...
#include "scriptGameSvrConnecter.h"
#include "gamesvr.h"
#include "gamelogic/UsrEngn.h"

void CScriptGameSvrConnecter::OnAsyncConnect(){
	FUNCTION_BEGIN;
...
void CScriptGameSvrConnecter::OnDisconnect(){
	FUNCTION_BEGIN;
	__super::OnDisconnect();
	m_bovalid = false;
}
bool CScriptGameSvrConnecter::isvalid(){
	FUNCTION_BEGIN;
	return m_bovalid;
}
time_t CScriptGameSvrConnecter::valid_timeout(){
	FUNCTION_BEGIN;
	return 30;
}

bool CScriptGameSvrConnecter::msgParse(stBaseCmd* pcmd, unsigned int ncmdlen,stQueueMsgParam* bufferparam){
	FUNCTION_BEGIN;
	switch(pcmd->value)
	{
	...
	default:
		{
			CUserEngine::getMe().doScriptSvrCmd(pcmd,ncmdlen);
		}
	}
	return true;
}

bool CScriptGameSvrConnecter::OnstSvr2SvrLoginCmd(stSvr2SvrLoginCmd* pcmd, unsigned int ncmdlen,stQueueMsgParam* bufferparam){
	FUNCTION_BEGIN;
	if (svr_id_type_value != pcmd->svr_id_type_value) {
		...
		m_bovalid = true;
		g_logger.debug("Script游戏服务器 %d(%d) 登陆校验成功!",svr_id,svr_type);
	}
	return true;
}
